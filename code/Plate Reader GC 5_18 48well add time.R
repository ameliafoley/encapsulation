...
#path to data
data_location3 <- here::here("data","growthcurve5_18_19_table_edited.xlsx")
...
ggplot(data = merge, aes(x = time_h, y = corrected, color = sample, shape = type)) +
  ...
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed")) +ggtitle("Rep2 in 48-Well Plate w/ Added Manual Time Points")

four<- subset(merge, volume == "400")
ggplot(data = four, aes(x = time_h, y = corrected, color = sample, shape = type)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed")) +ggtitle("Rep2 in 48-Well Plate w/ Added Manual Time Points (400uL)")

six<- subset(merge, volume == "600")
ggplot(data = six, aes(x = time_h, y = corrected, color = sample, shape = type)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed")) +ggtitle("Rep2 in 48-Well Plate w/ Added Manual Time Points (600uL)")
