...
phase1_time_map <- tibble::tribble(
  ...
  "p.putida", "LB", "cap", 0, 15, 110, NA, #high min for phase 2 since no phase 2 exists
  "p.putida", "LB", "chit", 0, 20, 110, NA, #high min for phase 2 since no phase 2 exists
  "p.putida", "sRB15", "cap", 0, 25, 25, NA,
  "p.putida", "sRB15", "chit", 0, 15, 15, NA
...
summarize_plate_two_phase_prejoined <- function(df,
  ...
  df %>%
    ...
    summarize(
      ...
      # --- Phase 2 detection (first local max after diauxie), with per-group min/max phase2 window ---
      {
        ...
        if (!is.na(diauxie_idx) && !is.na(first(treatment)) && (first(treatment) %in% two_phase_treatments)) {
          ...
          # apply per-group min/max phase2 restriction if present (joined as eff_min_phase2_time / eff_max_phase2_time)
          # Lower bound:
          ...
          if (!is.na(eff_max_phase2_time)) {
            max2_candidates <- max2_candidates[time_h[max2_candidates] <= eff_max_phase2_time]
          } else if (!is.null(max_phase2_time)) {
            # fallback to scalar argument if provided
            max2_candidates <- max2_candidates[time_h[max2_candidates] <= max_phase2_time]
          }
...
res_global_maxpercap <- run_global_anova(gc_test2, "max_percap")

# --- Run for multiple response variables in one go (vectorized) ---
vars_to_run <- c("max_percap1", "max_dens", "auc", "lag_time1", "max_percap2")  # adjust as needed
