...
f199_w<- f199 %>% dplyr::select (-OD600, -media, -strain, -treatment, -initial) %>% 
  pivot_wider(names_from = well, values_from = corrected) %>%
  filter(time_h < 113)

gc_out <- SummarizeGrowthByPlate(f199_w)
plot(gc_out)
#add sample info
...
both_w<- both %>% 
  filter(strain == "n.aroma" & time_h < 113 | 
         strain == "p.putida" & time_h <71) %>%
  dplyr::select (-OD600, -media, -strain, -treatment, -initial) %>% 
  pivot_wider(names_from = well, values_from = corrected)

gc_both <- SummarizeGrowthByPlate(both_w)
...
g7_w<- g7 %>% dplyr::select (-OD600, -media, -strain, -treatment, -initial) %>% 
  pivot_wider(names_from = well, values_from = corrected)%>%
  filter(time_h < 71)

gc_g7 <- SummarizeGrowthByPlate(g7_w)
...
g72_w<- g72 %>% dplyr::select (-OD600, -media, -strain, -treatment, -initial) %>% 
  pivot_wider(names_from = well, values_from = corrected)%>%
  filter(time_h < 71)

gc_g72 <- SummarizeGrowthByPlate(g72_w)
...
f1992_w<- f1992 %>% dplyr::select (-OD600, -media, -strain, -treatment, -initial) %>% 
  pivot_wider(names_from = well, values_from = corrected)%>%
  filter(time_h < 113)

gc_f1992 <- SummarizeGrowthByPlate(f1992_w)
...
clean_sum <- gc %>% dplyr::select(-sample) %>% group_by(media, strain, treatment) %>% summarize(mean_k = mean(k), se_k = std.error(k), 
