...
#path to data
data_location <- here::here("data","Ecoli_decap_7_18_plate.xlsx")
...
ggplot(data = df, aes(x = time, y = OD600, color = cell_load, shape = cell_load)) +
  ...
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed")) +ggtitle("E. coli Decapsulation")+ 
  facet_wrap(~alginate)
ggsave("ecoli_decap.png", width = 5, height = 4)
