...
#load data. 
consortia <- read_excel(here::here("data", "Exp 2.2 PAH Results copy.xlsx"))
consortia <- consortia[ , -c(6,7)]
consortia.code <- read_excel(here::here("data", "Exp 2.2 Sample Code copy.xlsx"))
...
lmm_posthoc_by_compound <- function(df, cmpd) {
  dat <- df %>%
    ...
    mutate(
      ...
      day = factor(day, levels = c("0","7","14","21")),  # enforce categorical days
...
emm.phe<- emmeans(model.phe, ~ treatment * consortia | day)  # now you should see day broken out
contrast(emm.phe, method = "pairwise", by = c("treatment","day")) #consortia differences, within each treatment and day
contrast(emm.phe, method = "pairwise", by = c("consortia","day")) #treatment differences, within each consortia and day

##NAP
model.nap <- lmm_posthoc_by_compound(long_with_flags, "naphthalene")
emm.nap<- emmeans(model.nap, ~ treatment * consortia | day)
contrast(emm.nap, method = "pairwise", by = c("treatment","day")) #consortia differences, within each treatment and day
contrast(emm.nap, method = "pairwise", by = c("consortia","day")) #treatment differences, within each consortia and day

##FLA
model.fla <- lmm_posthoc_by_compound(long_with_flags, "fluoranthene")
emm.fla<- emmeans(model.fla, ~ treatment * consortia | day)
contrast(emm.fla, method = "pairwise", by = c("treatment","day")) #consortia differences, within each treatment and day
contrast(emm.fla, method = "pairwise", by = c("consortia","day")) #treatment differences, within each consortia and day
