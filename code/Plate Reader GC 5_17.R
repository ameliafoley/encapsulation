...
#capitalize
graph_data<- graph_data %>% mutate(Strain = fct_recode(strain, 
                                                     "F199" = "f199", 
                                                     "G7" = "g7", 
                                                     "Rep2" = "rep2")) %>% 
  mutate(Sample = fct_recode(sample, 
                             "F199 Beads A" = "bf_22_a", 
                             "F199 Beads B" = "bf_22_b", 
                             "F199 Beads C" = "bf_22_c")) %>%
...
#control
ggplot(data = control, aes(x = time_h, y = OD600, color = sample, shape = type)) +
...
#play with facet wrap
ggplot(data = graph_data, aes(x = time_h, y = corrected, color = sample)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed")) + facet_wrap(~ type+strain)+ggtitle("All Samples")
ggplot(data = free, aes(x = time_h, y = corrected, color = sample)) +
  geom_point() + geom_smooth() + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))
...
ggplot(data = one, aes(x = time_h, y = corrected, color = sample, shape = type)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))+ggtitle("All Samples in 100uL")+ facet_wrap(~ type+strain)
...
ggplot(data = two, aes(x = time_h, y = corrected, color = sample, shape = type, shape = type)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))+ggtitle("All Samples in 200uL")+ facet_wrap(~ type+strain)
...
#CAPITALIZE
ggplot(data = beadstwof, aes(x = time_h, y = corrected, color = Sample)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))+ggtitle("F199 Beads in 200uL")
ggsave("f199beadsin200ul.png", width = 6, height = 4)
...
onerep2<- subset(one, strain =="rep2")
ggplot(data = onerep2, aes(x = time_h, y = corrected, color = sample, shape = type)) +
  geom_point() + geom_smooth(se = FALSE) + theme_classic() + xlab("Time (hr)") + ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))+ggtitle("Rep2 in 100 uL")
...
tworep2<- subset(two, strain == "rep2")
ggplot(data = tworep2, aes(x = time_h, y = corrected, color = sample, shape = type)) + 
  geom_point() + geom_smooth(se = FALSE) + 
  theme_classic() + 
  xlab("Time (hr)") + 
  ylab("OD600 (Corrected)") + 
  theme(panel.grid.minor.y = element_line(color = "grey", linetype = "dashed"))+
  ggtitle("Rep2 in 200 uL")
